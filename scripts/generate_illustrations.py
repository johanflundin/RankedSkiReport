...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as pool:
        for s in slots:
            ...
            # fresh BytesIO list per submission (file pointers are stateful)
            refs_copy = [bytes(b) for b in style_refs]
            futures[pool.submit(
                generate_one, client, s, full_prompt, size, args.quality, refs_copy, out_dir
            )] = s
